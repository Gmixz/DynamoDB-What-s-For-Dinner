...
import { RecipeDynamoModel } from '../models/recipe.dynamo.model';
...
export class RecipeManagerComponent implements OnInit {
  ...
  private dynamoDocument: DynamoDBDocument;

  public recipeName: string = '';
  public recipeDescription: string = '';
  public recipeIngredients: string[] = [];
  public recipeInstructions: string = '';
  public recipeTime: number = 0;

  public deleteRecipeName: string = '';


  constructor() {
    this.dynamodbClient = new DynamoDB({}); //TODO: Add credentials...
  ...
  async deleteExistingRecipe(): Promise<any>{   // Delete recipe Function
    ...
    await this.dynamoDocument.put({
      ...
      Item: {
        recipeId : '1',
        recipeSortKey: '1',
        payload: JSON.stringify(recipesToKeep),
      },
    });
  }


  async addRecipe(): Promise<any>{    // Add Recipe Function
    let existingRecipes = await this.getRecipes();
    ...
    recipeToAdd.ingredients = this.recipeIngredients;
    ...
    await this.dynamoDocument.put({
      ...
      Item: {
        recipeId : '1',
        recipeSortKey: '1',
        payload: JSON.stringify(existingRecipes),
      },
    });

  }

  async getRecipes(): Promise<Recipe[]> { //Get Recipe Function
    let queryParameters = {
      TableName: 'recipe-storage',
    ...
    let recipeDynamoModel : RecipeDynamoModel = JSON.parse(JSON.stringify(dataInDynamoDb.Item));
    
    return JSON.parse(recipeDynamoModel.payload) as Recipe[];
  }
